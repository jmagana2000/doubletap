...
- `ml/` — the recommendation engine:
  ...
  - `reward.py` — `PMIModel`: smoothed pairwise PPMI over deck co-occurrence — literally a card-synergy graph stored as a flat `{(a,b): score}` dict. Used as the CQL training reward, as swap-cut rationale (`swaps.py`), and (`make_pmi_reranker`) as an opt-in `--synergy-weight` reranker for `recommend`/`complete`.
